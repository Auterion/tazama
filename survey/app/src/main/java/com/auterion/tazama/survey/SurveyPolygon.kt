...
fun SurveyPolygon(
    ...
    onVertexAtIndexChanged: (Int, LatLng) -> Unit,
    onDeleteVertex: (Int) -> Unit
) {

    val pointsForPolyline: MutableList<LatLng> =
        vertices.sortedBy { it.sequence }.filter { it.role == VertexRole.DRAGGER }
        ...
        key(vertex.id) {
            CircleWithItem(
                center = when (vertex.role) {
                    ...
                    VertexRole.INSERTER -> DraggerCoordinateForId(
                        id = vertex.id,
                        vertices = vertices
                    )
                },
                ...
                onCenterChanged = { latLng ->
                    ...
                    onVertexAtIndexChanged(vertex.id, latLng)
...
@Composable
fun DraggerCoordinateForId(id: Int, vertices: MutableList<Vertex>): LatLng {
    val sequence = vertices.find { it.id == id }?.sequence?.let { sequence ->
        ...
        val pixelDragger =
            pixelPrev + (pixelNext - pixelPrev).apply { x *= 0.5f; y *= 0.5f }
